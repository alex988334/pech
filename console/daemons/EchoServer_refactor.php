...
class EchoServer extends WebSocketServer 
{
    ...
    protected function decodeRequest(string $json)
    {
        $this->request = json_decode($json, true);  
        
        switch ($this->request['action']){            
            case 'blackListUsers': $this->request['operation'] = self::OP_BLACK_LIST_USERS;
                break;
            case 'blockUsers': $this->request['operation'] = self::OP_BLOCK_USERS;
                break;
            case 'createChat': $this->request['operation'] = self::OP_CREATE_NEW_CHAT;
                break;            
            case 'exitChat': $this->request['operation'] = self::OP_EXIT_CHAT;
                break;
            case 'getChats': $this->request['operation'] = self::OP_GET_CHATS;
                break;
            case 'historyChat': $this->request['operation'] = self::OP_GET_HISTORY_MESSAGE;
                break;            
            case 'addUserInChat': $this->request['operation'] = self::OP_LIST_USERS;
                break;
            case 'newMessage': $this->request['operation'] = self::OP_NEW_MESSAGE;
                break;
            case 'removeChat': $this->request['operation'] = self::OP_REMOVE_CHAT;
                break;
            case 'deleteUserFromChat': $this->request['operation'] = self::OP_REMOVE_USER;
                break;
            case 'addUserInChat': $this->request['operation'] = self::OP_ADD_USER;
                break;
            case 'searchUser': $this->request['operation'] = self::OP_SEARCH_USER;
                break;
            case 'setName': $this->request['operation'] = self::OP_SET_USER_NAME;
                break;
            case 'status': $this->request['operation'] = self::OP_STATUS_MESSAGE;
                break;
            case 'system': $this->request['operation'] = self::OP_SYSTEM;
                break;
            default : throw new \yii\web\BadRequestHttpException('Error "action"!');
        }
    }
    
    protected function securityClient(ConnectionInterface $client)
    {
        $this->client = $client;
        
        if ($this->request['action'] == 'setName') {
            return;
        }
        if (!isset($client->name) || !isset($client->idUser)) {            
            throw new Exception('Error: bad client parametrs!');
        }
    }
    ...
    {    
        $user = User::find()->where(['username' => $client->name])->joinWith('role')->asArray()->one();   
        if (!$user) {
            throw new \yii\db\Exception('Error: not found user in data base!');
        }
        
        if ((isset($client->idUser) && $user['id'] == $client->idUser) || $user) {   
            $client->role = $user['item_name'];
            return $user['id'];
        } 
    ...
    protected function securityRequest()
    {      
        switch($this->request['operation']){
            case self::OP_SYSTEM:                 
                if (empty($this->request['id'])){ 
                    $this->error[] = 'Не установлен id';                    
                }
                if (empty($this->request['status'])) {
                    $this->error[] = 'Не установлен status';                    
                }                
                break;
            case self::OP_NEW_MESSAGE:               
                if (empty($this->request['id_chat'])){
                    $this->error[] = 'Не установлен id';
                }                
                if (empty($this->request['message'])){
                    $this->error[] = 'Не установлен message';
                }    
                if (!Chat::findOne(['id_chat' => $this->request['id_chat']])) {
                    $this->error[] = 'Чат не найден';
                }
                if (!ChatUser::findOne(['id_chat' => $this->request['id_chat'], 'id_user' => $this->client->idUser])) {
                    $this->error[] = 'Вы не участник чата';
                }
                break;
            case self::OP_SET_USER_NAME: 
                if (empty($this->request['name']) || trim($this->request['name'])){
                    $this->error[] = 'Не установлен name';
                }                                              
                break;                 
        }
        
        if (count($this->errors)){
            throw new \yii\web\BadRequestHttpException('Error: bad request parametrs!');
        }
    }
    
    protected function securityDB()
    {
        try {
            ...
            Yii::$app->db->close(); 
            $this->controller->log('SQL CONNECTION STATUS -> CLOSED', ServerController::LOG_ERROR);            
            try {                
                Yii::$app->db->open();
                $this->controller->log('SQL CONNECTION, STATUS -> OPENED', ServerController::LOG_ERROR);
            } catch (\yii\db\Exception $exception) {                
                throw new \yii\db\Exception('Error: database connect fuirlure!');
            }
        }  
    }
    ...
    public function commandSetName(ConnectionInterface $client, $msg)
    {
        ...
        $this->controller->log('total = ' . $this->clients->count(), ServerController::LOG_WARNING);      

        if (empty($this->request['name']) || !$name = trim($this->request['name'])) { 
        ...
        $client->name = $name;        
        try {
            $client->idUser = $this->securityUser($client);
        } catch (\yii\db\Exception $ex) {
            $client->name = NULL;
            $client->close();
        }
        
        ChatUser::updateAll(['client_hash' => $this->clients->getHash($client)], ['id_user' => $client->idUser]);
    ...
    {          
        try {
            $this->decodeRequest($msg);        
            $this->securityClient($from);
            ...
            $this->errors[] = implode(', ', $exception->errorInfo);
            $this->sendError();
            $from->close();
    ...
    protected function createNewChat(int $idAutor, array $idUsers, string $alias, int $status = Chat::CHAT_ACTIVE)
    {
        $chat = new Chat(['autor' => $idAutor, 'alias' => $alias, 
                            'create_at' => date('Y-m-d'), 'status' => $status]);
        foreach ($idUsers as $v) {
            $mass[] = [$chat->id, $v];
        }
        if ($chat->save()) {
            Yii::$app->db->createCommand()->batchInsert('chat_user', ['id_chat', 'id_user'], $mass)->execute();
            $this->selectUsers($mass);
            return $chat;
        } 
        
        throw new \yii\db\Exception('Error: fairlure save new chat in database!');   
    }
    /** 
     * @param int $idChat
     * @param array $users
     ...
     *  ]
     */
    protected function selectSendUsers(array $usersId)
    {
        if (isset($usersId)) {
            $where = ['id_user' => $usersId];
        } else {
            $where = ['id_chat' => $this->request['idChat']];
        }
        $users = ChatUser::find()->select(['idUser' => 'id_user', 'clientHash' => 'client_hash'])
                    ->where($where)->indexBy('idUser')->asArray()->all();  
        if (!count($users)) {
            throw new \yii\web\NotFoundHttpException('Error: not found users in chat!');
        }  
    }
    
    protected function selectUsers(array $idUsers)
    {       
        $listUsers = Master::find()->select(['login' => 'username', 'id' => 'id_master', 
                    'fio' => 'CONCAT(familiya, " ", imya, " ", otchestvo)'])
                ->where(['id_master' => $idUsers])->join('INNER JOIN', 'user', 'user.id=master.id_master')               
                ->asArray()->all();          
        foreach ($listUsers as $one) $result[] = $one;

        $listUsers = Manager::find()->select(['login' => 'username', 'id' => 'id_manager', 
                    'fio' => 'CONCAT(familiya, " ", imya, " ", otchestvo)'])
                ->where(['id_manager' => $idUsers])->join('INNER JOIN', 'user', 'user.id=manager.id_manager')                
                ->asArray()->all();
        foreach ($listUsers as $one) $result[] = $one;

        $listUsers = Klient::find()->select(['login' => 'username', 'id' => 'id_klient', 
                    'fio' => 'CONCAT(familiya, " ", imya, " ", otchestvo)'])
                ->where(['id_klient' => $idUsers])->join('INNER JOIN', 'user', 'user.id=klient.id_klient')                
                ->asArray()->all();
        foreach ($listUsers as $one) $result[] = $one;
        ...
        $this->request['users'] = ArrayHelper::index($result, 'id');
    }


    protected function createNewMessage(int $idChat, int $idAutor, string $message)
    {
        $message = new ChatMessage(['id_chat' => $idChat, 'id_user' => $idAutor, 'message' => $message,
                'date' => date('Y-m-d'), 'time' => date('H:i:s')]); 
        if ($message->save()) {
            $date = date('Y-m-d');
            $time = date('H:i:s'); 
            $this->request['messages'] = [$message->attributes];
            foreach ($this->request['users'] as $one) {
                $mass[] = [$message->id, $one['id'], self::MESSAGE_SEND, $date, $time];
            }
            try {
                Yii::$app->db->createCommand()->batchInsert('chat_message_status', 
                        ['id_message', 'id_user', 'status_message', 'date', 'time'], $mass)->execute();
            } catch (\yii\db\Exception $e) {
                throw new \yii\db\Exception('Error: update status message fairlure!');
            }
            return;
        }        
        
        throw new \yii\db\Exception('Error: fairlure save new message in database!');   
    } 
    
        
    protected function sendMessage(int $status)
    {
        $data = ['chats' => $this->request['chats'], 'messages' => $this->request['messages'],
                'users' => $this->request['users']];
        if ($this->request['status'] == self::STATUS_ACCEPT){            
            $data['status'] = new \common\models\responseData\Status(['status' => self::STATUS_ACCEPT,
                    'operation' => $this->request['operation']]);
        }
        
        if ($status == self::STATUS_ERROR) {
            $data['status'] = new \common\models\responseData\Status(['status' => self::STATUS_ERROR,
                    'operation' => $this->request['operation']]);               
        }   
        
        if ($this->request['operation'] == self::OP_STATUS_MESSAGE) {
    ...
    protected function createSystemMessage(int $idUser, string $fio, int $idOrder)
    {
        switch ($this->request['status']) {
            case \common\models\VidStatusZakaz::ORDER_REQUEST_TAKE: 
                    $strM = 'Запрос мастера №'. $idUser
                   . ' '. $fio . ' на взятие заявки №' . $idOrder;
                break;
            case \common\models\VidStatusZakaz::ORDER_REQUEST_REJECTION:
                    $strM = 'Запрос мастера №'. $idUser 
                    . ' '. $fio . ' на отказ от заявки №' . $idOrder;
                break;
            case \common\models\VidStatusZakaz::ORDER_REQUEST_EXECUTE:
                    $strM = 'Запрос мастера №'. $idUser
                    . ' '. $fio . ' о готовности заявки №' . $idOrder;
                break;    
            default :  $strM = '';
        }
        return $strM;
    ...
    public function commandSystem(ConnectionInterface $client, $msg)
    {
        ...
        try {            
         /*   $master = Yii::$app->db->createCommand('SELECT id_master, CONCAT(familiya, " ", imya, " "'
            ...
            */
            $this->selectUsers([$client->idUser]);
            ...
            if (!$chatId) {
                $chat = $this->createNewChat(0, [0, $managerId], 'System');                
            }             
            
            $this->selectSendUsers();dffdg
            
            $message = $this->createNewMessage($chatId ?? $chat->id, 0, 
                    $this->createSystemMessage($master['id_master'], $master['fio'], $this->request['id']));
            
            $send = ChatMessageStatus::MESSAGE_SEND;
            foreach ($this->clients as $one) {
                if ($one->idUser == $managerId) {
                    if (!$chatId) {
                        $this->request['operation'] = self::OP_CREATE_NEW_CHAT;
                        $this->sendMessage(self::STATUS_ACCEPT, [$chat->id => $chat->attributes], [], []);              
                    }
                    $this->request['operation'] = self::OP_NEW_MESSAGE;
                    $this->createSendMessage(self::STATUS_ACCEPT, [], [$message->id => $message->attributes], []); 
                    $send = ChatMessageStatus::MESSAGE_DELIVERED;
                    break;
                }
            }
            $this->updateStatusMessage($idChat)
            ...
            $transact->rollBack();
            print_r($ex);
            $this->errors[] = $ex->getMessage() . '; ' . $ex->getTraceAsString();
            $this->sendError();
            return;
        }
   ...
   /* protected function securityMessage(ConnectionInterface $client, string $msg)
     ...
     * @return type
     */
    public function saveNewMessageData()
    {
        $this->controller->log('saveMessage -> START', ServerController::LOG_WARNING);        
        
        $this->request['idAuthor'] = $this->client->idUser;
        $this->request['autor'] = $this->client->name; 
        
        if (!$newMessage = $this->createNewMessage($this->request['idChat'], 
                    $this->request['id_autor'], $this->request['message'])) {
            debugArray($newMessage);
            return NULL;
        }
...
/*	if ($usersNames != null) $this->controller->log('sendNewMessage -> $usersNames -> ' . implode(', ', $usersNames));
	else $this->controller->log('sendNewMessage -> $usersNames = null');

	if ($newMessage != null) $this->controller->log('sendNewMessage -> $newMessage -> ' .implode(', ', $newMessage->getAttrOfArray()));           
	else $this->controller->log('sendNewMessage -> newMessage = null');
*/
        $newMessage->operation = self::OP_NEW_MESSAGE;
     ...
     * @return type
     */
    public function commandNewMessage(ConnectionInterface $client, $msg)
    {
        $this->controller->log('commandChat -> START', ServerController::LOG_WARNING);      
        
        try {
            $this->saveNewMessageData();
...
UPDATE chat_message_status ms, chat_user u SET status_message='delivered' WHERE u.id_user=ms.id_user AND ms.id_user=380 AND u.id_chat=27  
    ...
    {        
        ...
        if (!isset($request['id'])) {
            $client->send(json_encode(['operation' => self::OP_LIST_USERS, 'status' => self::STATUS_ERROR, 
                                            's_message' => 'Не заданы параметры']));
        ...
        try {
            ...
            $blocked = ArrayHelper::getColumn($blocked, 'locked');
            reset($roles);
            ...
            while ($one = current($roles)){
                ...
                $roles[$key]['connected'] = false;
                
                if (in_array($roles[$key]['user_id'], $blocked)) $roles[$key]['blocked'] = true;
                else $roles[$key]['blocked'] = false;
                
                foreach ($this->clients as $oneClient){
                    if ($oneClient->idUser == $one['user_id']) {
                        $roles[key($roles)]['connected'] = true; 
                        break;
                    }
                }                
                next($roles);
            ...
            $client->send(json_encode(['operation' => self::OP_LIST_USERS, 'status' => self::STATUS_ERROR,
                                'users' => $users, 's_message' => 'Ошибка поиска в бд']));
            $this->controller->log('commandListChatUsers EROR Ошибка поиска в бд', ServerController::LOG_ERROR);
            $this->controller->log('commandListChatUsers -> END', ServerController::LOG_WARNING);
            return;            
        }
        
        $client->send(json_encode(['operation' => self::OP_LIST_USERS, 'status' => self::STATUS_ACCEPT, 'users' => $roles,
                                'id' => $client->idUser, 'autor' => $users[key($users)]['autor']]));
    ...
    public function commandRemoveChat(ConnectionInterface $client, $msg)
    {
        ...
        $client->send(json_encode(['status' => self::STATUS_ERROR, 'operation' => self::OP_REMOVE_CHAT, 
                        's_message' => 'Ошибка базы данных, повторите операцию']));
    ...
    public function commandBlackListUsers(ConnectionInterface $client, $msg)
    {
        ...
        if (!$lockUsers){                                                               //  проверяем наличие заблокированных
            $client->send(json_encode(['status' => self::STATUS_ACCEPT, 'operation' => self::OP_BLACK_LIST_USERS, 
        ...
        try {
            foreach ($roles as $key => $one){
                switch ($one['item_name']){
                    ...
                    case AuthItem::KLIENT: $table = 'klient';
                        $field = 'id_klient';                    
                        break;
                    default: return $client->send(json_encode(['status' => self::STATUS_ERROR, 'operation' => self::OP_BLACK_LIST_USERS, 
                                                        's_message' => 'Не опознанный пользователь!!!']));
                }
                $query = Yii::$app->db->createCommand('SELECT username, CONCAT(p.familiya, " ", p.imya, " ", p.otchestvo) AS fio '
                        . 'FROM '. $table .' p, user u WHERE p.'. $field .'=u.id AND u.id=' . $one['user_id'])->queryOne();
            ...
            $client->send(json_encode(['status' => self::STATUS_ERROR, 'operation' => self::OP_BLACK_LIST_USERS, 
                                            's_message' => 'Ошибка поиска в бд']));
