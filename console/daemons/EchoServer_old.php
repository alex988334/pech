...
class EchoServer_old extends WebSocketServer
{
    ...
    public function commandSystem(ConnectionInterface $client, $msg)
    {
        ...
        if (!isset($request['id']) || !isset($request['status'])) return;
    ...
    protected function sendNewMessage(array $usersNames, Message $newMessage)
    {
        ...
        print_r($usersNames);
        print_r($newMessage);
    //    $this->controller->log('sendNewMessage -> $usersNames -> ' . implode(', ', $usersNames));
     //   $this->controller->log('sendNewMessage -> $newMessage -> ' . implode(', ', $newMessage->getAttrOfArray()));
        $this->clients->count();
        
        $newMessage->operation = self::OP_INPUT_MESSAGE;
        $newMessage->status = self::STATUS_ACCEPT;
        $newMessage->s_code = self::MESSAGE_SEND;
        ...
        try{
            ...
            foreach ($messages as $one) {
                ...
                if ($block) {                             
                    ...
                    switch ($status) {
                        case ChatMessageStatus::MESSAGE_SEND: $status = self::MESSAGE_SEND;
                            break;
                        case ChatMessageStatus::MESSAGE_DELIVERED: $status = self::MESSAGE_DELIVERED;
                            break;
                        case ChatMessageStatus::MESSAGE_READED: $status = self::MESSAGE_READED;
                            break;
                    }
...
UPDATE chat_message_status ms, chat_user u SET status_message='delivered' WHERE u.id_user=ms.id_user AND ms.id_user=380 AND u.id_chat=27  
    ...
    public function commandSetName(ConnectionInterface $client, $msg)
    {
        ...
        $client->name = $name;
      //  if (isset($request['imei']) && $imei = trim($request['imei'])) $client->imei = $imei;
