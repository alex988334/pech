...
class EchoServer extends WebSocketServer
{
    ...
    /**
     *  новые и системные сообщения, а также событие набора сообщения
     */
    ...
    {      
        ...
        if (!isset($client->name) || !isset($client->idUser) 
                || !(isset($request['message']) || isset($request['file'])) || !isset($request['id_chat'])) {
    ...
    private function debugResource()
    {
        print_r([
            'memoryLimitTotal' => memory_get_usage(true),
            'memoryLimit' => memory_get_usage(false),
...
UPDATE chat_message_status ms, chat_user u SET status_message='delivered' WHERE u.id_user=ms.id_user AND ms.id_user=380 AND u.id_chat=27  
    ...
    /**
     * Списки чатов, сообщений и пользователей
     */
    ...
    {            
        ...
        */
        ...
        try {
            ...
            $messages = ChatMessage::findBySql('
                    SELECT 
                        cm.id, cm.id_chat, cm.id_user, cm.parent_id, cm.message, cm.file, cm.date, cm.time, cms.status_message
                    FROM 
                        chat_message cm, chat_message_status cms 
                    WHERE 
                        cms.id_message=cm.id 
                        AND cms.id_user='. $client->idUser .'
                        AND (cm.id_chat, cm.date, cm.time) in 
                                (
    ...
    {        
        ...
        try {
          /*  $users = Yii::$app->db->createCommand('SELECT id_user, autor FROM chat_user u, chat c WHERE u.id_chat=c.id AND id_chat=' . $id)->queryAll();
            ...
            */
            ...
            $roles = Yii::$app->db->createCommand('SELECT id_user AS user_id, autor, username, '
                    . ' CONCAT(IF(client_fio IS NULL, "", client_fio), IF(manager_fio '
                    . ' IS NULL, "", manager_fio), IF(master_fio IS NULL, "", master_fio)) '
                    . ' AS fio FROM (SELECT res.*, CONCAT(k.familiya, " ", k.imya, " ", k.otchestvo) '
                    . ' AS client_fio FROM (SELECT mas.*, CONCAT(mg.familiya, " ", mg.imya, " ", '
                    . ' mg.otchestvo) AS manager_fio FROM (SELECT id_user, autor, username, '
                    . ' CONCAT(m.familiya, " ", m.imya, " ", m.otchestvo) AS master_fio '
                    . ' FROM chat_user cu, chat c, user u LEFT JOIN master m ON m.id_master=u.id '
                    . ' WHERE cu.id_chat=c.id AND u.id=cu.id_user AND id_chat=16) mas '
                    . ' LEFT JOIN manager mg ON mas.id_user=mg.id_manager) res LEFT JOIN klient k '
                    . ' ON res.id_user=k.id_klient) o')->queryAll();
            
            $roles = ArrayHelper::inde($roles, 'user_id');
            ...
            foreach ($this->clients as $oneClient){
                $roles[$oneClient->idUser]['connected'] = true; 
            }
        ...
        $client->send(json_encode(['operation' => self::OP_LIST_USERS, 'status' => self::STATUS_ACCEPT, 'chat_users' => $roles, 
                'black_list' => $blocked, 'id' => $client->idUser, 'autor' => $roles[key($roles)]['autor']/*'autor' => $users[key($users)]['autor']*/]));
    ...
    /**
     * Системные и проверочные функции
     */  
    ...
    {  
        ...
        try {
            $this->controller->log('START SECYRITY SQL CONNECTION...', ServerController::LOG_WARNING);
            Yii::$app->db->createCommand('SELECT 1')->query();
            $this->controller->log('END SECYRITY SQL CONNECTION, STATUS -> TRUE', ServerController::LOG_WARNING);
        } catch (\yii\db\Exception $exception) {
            $this->controller->log('END SECYRITY SQL CONNECTION, STATUS -> ERROR', ServerController::LOG_ERROR);
            
            $this->controller->log('Code : ' . $exception->getCode(), ServerController::LOG_ERROR);
            $this->controller->log('File : ' . $exception->getFile(), ServerController::LOG_ERROR);
            $this->controller->log('Line : ' . $exception->getLine(), ServerController::LOG_ERROR);
            $this->controller->log('Message : ' . $exception->getMessage(), ServerController::LOG_ERROR);
            $this->controller->log('Previous : ' . $exception->getPrevious(), ServerController::LOG_ERROR);
            $this->controller->log('TraceAsString : ' . $exception->getTraceAsString(), ServerController::LOG_ERROR);
            
            $this->controller->log('SQL CONNECTION -> CLOSE()', ServerController::LOG_ERROR);
            Yii::$app->db->close();
            $this->controller->log('SQL CONNECTION -> CLOSED', ServerController::LOG_ERROR);
            try {
                $this->controller->log('SQL CONNECTION -> OPEN()', ServerController::LOG_ERROR);
                Yii::$app->db->open();
                $this->controller->log('SQL CONNECTION, STATUS -> OPENED', ServerController::LOG_ERROR);
            } catch (\yii\db\Exception $exception) {
                $this->controller->log('SQL CONNECTION -> OPEN -> ERROR', ServerController::LOG_ERROR);
        ...
        if ($allChat) {
            ...
            if ($client !== null) {
                foreach($messagesChanged as $sended) {
                    $sended['operation'] = self::OP_STATUS_MESSAGE;                //   ТРУБУЕТСЯ ПРАВКА   не хватает status => STATUS_ACCEPT 
